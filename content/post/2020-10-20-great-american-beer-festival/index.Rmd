...
We can aggregate `beer category` to `beer style` as defined by [Beer Advocate](https://www.beeradvocate.com/beer/styles/). This larger set of beers has only `r length(unique(beer_awards_joined$style))` different buckets and has on average `r round(mean(table(beer_awards_joined$style)), 2)` beers in each style.
