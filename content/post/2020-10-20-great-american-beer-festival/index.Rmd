...
1. Is there and to what level of geo-aggregation are breweries specializing in `beer styles` for award winning beers?
2. Are recent changes in popularity of awarded beers being driven by `brewery location`, `beer styles` or both?
...
In short, we see not only evidence of state specialization but also regional specialization. The West Coast with California, Oregon, and Washington all have over representation `Strong Ales`, and `Stouts`, while under representing `Pale Lagers`, and `Dark Lagers`. Only New York also over represents `Stout` beer style. The Midwest to Appalachian region, are more varied in terms of what `beer styles` are represented, however, `Dark Lagers` and `Pale Lagers` have a strong presences. Texas has a similar profile of awarded beer styles to the Midwest than the West Coast.

Lastly, we can regress number of specific beer style with year to see if any beer styles are becoming more popular over time or if any state is becoming more popular.
...
`Wild/Sour Beers` are a clear up-and-coming beer award winners. An interesting note is that if we return to the previous figure, `Wild/Sour Beers` have marginal state level over representation throughout the US. Similarly, `Specialty Beer` is generally not under nor over represented anywhere except where it's under represented in California and in Oregon. On the other hand, `India Pale Ales` seems mostly driven by breweries in California.
